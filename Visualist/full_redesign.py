...
class App(Tk):
    ...
    def init_main(self):
        ...
        self.dir = "gifblack"
        self.stock_bg = '#eff0f1'#eff0f1
        self.second_bg = "black"
        self.font = "courier"
        self.format = 'gif'

        # стили
        self.styler = ttk.Style()
        self.styler.configure("TButton", foreground="white", background="white", relief="flat")
        ...
        # меню
        menubar = Menu(self)
        self.filemenu = Menu(menubar, tearoff=-2)
        ...
        menubar.add_command(label='Новый холст', command=self.notebook.create_new_canvas)
        menubar.add_command(label='Вернуть хост', command=lambda:self.notebook.reestablish_tab())
        menubar.add_command(label='Инфо', command=lambda:self.notebook.image_processing.get_info())
        ...
        self.btn_fill = ttk.Button(self.toolbar_1, style="TButton", image=self.im_fill,
                                   command=lambda c=self.color_code[1]:
        ...
        self.thickness_value = Label(self.thickness_labelframe, text=0, anchor=N, bg=self.stock_bg,
                                     foreground=self.second_bg,
                                     font=("Comic Sans MS", 15, "bold"))
        self.thickness_scale = ttk.Scale(self.thickness_labelframe, orient=VERTICAL, length=50, from_=5, to=20,
        ...
        self.btn_arrow = ttk.Button(self.figure_labelframe, image=self.pic_arrow, style="TButton",
                                    command=lambda t=core.DEFAULT_THICKNESS, clr=core.DEFAULT_FIRST_COLOR:
                                    self.notebook.events.event_btnCreateVector(thickness=t, color=clr)
                                    )
        ...
        self.filter_2_percent = ttk.Scale(self.frame_filter2, length=150, from_=0, to=500, command=slider_filter2)
        self.filter_2_percent.set(250)
        ...
        self.r_scale_box = ttk.Scale(self.frame_filter4, length=130, from_=0, to=200, orient=HORIZONTAL, command=slider_filter_r)
        self.r_scale_box.set(100)
        self.g_scale_box = ttk.Scale(self.frame_filter4, length=130, from_=0, to=200, orient=HORIZONTAL, command=slider_filter_g)
        self.g_scale_box.set(100)
        self.b_scale_box = ttk.Scale(self.frame_filter4, length=130, from_=0, to=200, orient=HORIZONTAL, command=slider_filter_b)
        ...
        self.thickness_value.grid(row=0, column=2, rowspan=2, sticky="WE")
